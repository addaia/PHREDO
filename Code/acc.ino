...
float calibrateAccelerometer() {
  ...
  Serial.println("Calibrating accelerometer... Please keep the device stationary.");
  for (int i = 0; i < calibrationSamples; i++) {
    imu.read();
    bias += imu.a.x;
    delay(10);  // Adjust delay if needed for a stable reading
  }
  ...
  while (sqrt(pow(pose.x, 2) + pow(pose.y, 2)) < targetDistance) {
    if (millis() - lastUpdateTime >= updateInterval) {
      ...
      if (entryCount < MAX_ENTRIES) {
        imu.read();
        // Subtract bias from the raw reading
        float raw_ax = imu.a.x;
        float corrected_ax = raw_ax - ax_bias;
        // Normalize the reading (e.g., to convert mg to g)
        float normalized_ax = corrected_ax / NORMALIZATION_FACTOR;
        // Filter the reading to smooth out noise
